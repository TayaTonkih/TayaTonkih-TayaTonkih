# 👋 Привет, я Таисия!

### 🎓 Frontend/Backend Developer

> Студентка 4 курса Самарского университета им. Королёва.  
> Специализируюсь на разработке современных SPA-приложений, Backend разработкой на C#/Java,Python и также интересуюсь ИИ сферой .

---

### 🚀 Мои навыки

| Категория | Технологии |
|-----------|-----------|
| 💻 Языки | TypeScript, Python, C#/Java, HTML/CSS |
| ⚙️ Фреймворки | Angular, Bootstrap, Git/GitHub |
| 🗄️ Базы данных | PostgreSQL, MySQL |
| 🧠 Computer Science | ООП, проектирование БД, UML, BPMN |

---
> Три года активно работаю с нейросетями — тестирую, анализирую ответы, нахожу ошибки. Умею критически оценивать тексты, проверять факты и быстро разбираться в незнакомых темах. 
> Имею опыт научно-исследовательской работы и написания академических текстов.
### 💼 Проекты

#### 🍰 «Торты на заказ» (SPA)
> Клиентская часть на Angular: роутинг, компонентная архитектура.  
> 🔗 [Посмотреть код](https://github.com/TayaTonkih/Project_Tonkih_6205) | 🌐 [Демо](https://TayaTonkih.github.io/cakes/)

#### 🏨 АИС учёта проживающих
> Проектирование реляционной БД (PostgreSQL), UML и BPMN-диаграммы
> Разрабатываю backend-логику на C#  

#### 💜 Моё портфолио
> Интерактивный сайт-резюме с проектами и контактами  
> 🔗 [Посмотреть код](https://github.com/TayaTonkih/taya-portfolio) | 🌐 [Открыть сайт](https://TayaTonkih.github.io/taya-portfolio/)

---

### 📫 Контакты

[![Email](https://img.shields.io/badge/Email-taya.tonkih@mail.ru-pink?style=for-the-badge&logo=gmail)](mailto:taya.tonkih@mail.ru)
[![GitHub](https://img.shields.io/badge/GitHub-@TayaTonkih-purple?style=for-the-badge&logo=github)](https://github.com/TayaTonkih)

---

<p align="center">
  <em>🌸 Всегда учусь новому и ищу интересные проекты для развития!</em>
</p>
